...
/// <summary>
/// Development seed data. Four surveys of deliberately different shapes, because the
/// object model claims to serve sprint retros, group decisions, company-wide feedback and
/// personal journals. If it only really fits the first one, better to find out here than
/// after building the admin UI.
/// </summary>
public static class SeedData
{
    ...
    /// <summary>
    /// The canonical case: a dozen named responses with heavy theme overlap and at least
    /// one genuine causal chain running through them (slow CI leads to batched commits
    /// leads to larger PRs leads to slower reviews). Closed, so it can be summarised.
    /// </summary>
    private static Survey Retro() => Build(
        "spr47ab", "dev-retro",
        "Sprint 47 retro",
    ...
    /// <summary>
    /// The small, low-stakes case. Five responses, one of them a single word. Exists to
    /// check the summariser degrades gracefully rather than manufacturing five themes out
    /// of a lunch order.
    /// </summary>
    private static Survey Takeaway() => Build(
        "lunch42", "dev-lunch",
    ...
    /// <summary>
    /// The scale case: anonymous, closed, many topics, and a realistic proportion of
    /// low-effort answers. Real surveys are mostly noise with signal scattered through
    /// them, and a summariser that only works on carefully written paragraphs is not much
    /// use. Anonymous also means no timestamps exist at all, so this is where response
    /// ordering falls back to the random Guid.
    /// </summary>
    private static Survey CompanyWide() => Build(
        "allco26", "dev-company",
    ...
    /// <summary>
    /// The single-author case, and the one most likely to break assumptions. Author is a
    /// date rather than a person, which is a legitimate stretch of a self-declared free
    /// text field and a useful check that nothing assumes Author means human.
    /// </summary>
    private static Survey Diary() => Build(
        "diary08", "dev-diary",
        "August journal",
